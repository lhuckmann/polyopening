...
class PolyhedralOpeningAngle:
    ...
    atom_type : str, optional
        ...
        should be evaluated. Default is "C".
    """
    ...
    def compute(
        ...
        if "all" in self.atom_type:
            ...
            idx=np.where(np.isin(elem, self.atom_type))[0]
            xyz=coords[idx]
        i, j = neighbor_list("ij", atoms, self.cutoff)
        
        PHI=[]
        for refidx, refatom in enumerate(xyz):
            phi=self._analyze_cp(atoms,refatom,i,j,refidx)
            PHI.append(phi)
        return np.array(PHI)
